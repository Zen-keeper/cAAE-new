...
def encoder(x, z_dim, reuse=False, is_train=True):
    ...
    with tf.variable_scope("Encoder", reuse=reuse):
        ...
        w_init = tf.truncated_normal_initializer(stddev=0.02)#从截断的正态分布中输出随机值
        b_init = tf.constant_initializer(value=0.0)
        gamma_init = tf.random_normal_initializer(1., 0.01)

        inputs = InputLayer(x, name='e_inputs')
        conv1 = Conv2d(inputs, gf_dim, (ft_size, ft_size), act=tf.nn.leaky_relu,
                       padding='SAME', W_init=w_init, b_init=b_init, name="e_conv1")
        # conv1_1 = tf.layers.batch_normalization(conv1,gamma_initializer=gamma_init,trainable=is_train,name='e_bn1')
        conv1 = BatchNormLayer(conv1, act=tf.nn.leaky_relu, is_train=is_train,
                               gamma_init=gamma_init, name='e_bn1')
        # conv1 = tf.nn.leaky_relu(conv1,name='e_bn1_relu')
        ...
        # # s16*s16
        # res5 = ResBlockDown(res4, gf_dim * 8, "res5", reuse, is_train)
        h_flat = tf.reshape(res3, shape=[-1, (int)(s4/2 * s4/2 * gf_dim * 8)])
...
def decoder(x, image_size=128,reuse=False, is_train=True):
    """
    ...
    with tf.variable_scope("Decoder", reuse=reuse):
        ...
        # s16*s16
        z_develop = DenseLayer(inputs, int(s4/2 * s4/2 * gf_dim * 8), act=lambda x : tf.nn.leaky_relu(x, 0.2),
                               name='g_dense_z')
        z_develop = tf.reshape(z_develop.outputs, [-1, int(s4/2), int(s4/2), gf_dim * 8])#原来都是16
        z_develop = InputLayer(z_develop, name='g_reshape')
        conv1 = Conv2d(z_develop, gf_dim * 8, (ft_size, ft_size), act=lambda x : tf.nn.leaky_relu(x, 0.2),
                       padding='SAME', W_init=w_init, b_init=b_init, name="g_conv1")

        # s16*s16
        res0 = ResBlockUp(conv1.outputs, s16*2, batch_size, gf_dim * 8, "gres0", reuse, is_train)
        # res1 = ResBlockUp(res0, s16, batch_size, gf_dim * 8, "gres1", reuse, is_train)

        # s8*s8
        res2 = ResBlockUp(res0, s8*2, batch_size, gf_dim * 4, "gres2", reuse, is_train)

        # s4*s4
        res3 = ResBlockUp(res2, s4*2, batch_size, gf_dim * 2, "gres3", reuse, is_train)

        # s2*s2
        # res4 = ResBlockUp(res3, s2*2, batch_size, gf_dim, "gres4", reuse, is_train)

        # image_size*image_size
        res_inputs = InputLayer(res3, name='res_inputs')
...
def discriminator(x, reuse=False):
    ...
    with tf.variable_scope("Discriminator", reuse=reuse):
        ...
        net_h0 = DenseLayer(net_in, n_units=n_l1,
                            W_init=w_init,
                            act=lambda x : tf.nn.leaky_relu(x, 0.2), name='dc/h0/lin')
        net_h1 = DenseLayer(net_h0, n_units=n_l2,
                            W_init=w_init,
                            act=lambda x : tf.nn.leaky_relu(x, 0.2), name='dc/h1/lin')
