...
def train(z_dim=None, model_name=None):
    ...
    with tf.device("/gpu:0"):
        ...
        encoder_output_test = encoder(x_test_input, z_dim, reuse=True, is_train=False)
        ...
        decoder_output_test, std_ = decoder(encoder_output_test, reuse=True, is_train=False)
        ...
        ssim_test = tf.reduce_mean(tf.image.ssim(x_test_input, decoder_output_test, max_val=1))
        ...
        with tf.device("/gpu:0"):
            ...
            with tf.control_dependencies(update_ops):
                ...
                autoencoderl2_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate_G,
                    ...
                    dc_loss + 10* autoencoder_loss + 0.5 *encoder_l2loss,var_list=[en_var,decoder_var])#,global_step=global_step
    ...
    for epoch in range(EPOCHS):
        ...
        data= datasets.create_datasets(retrain=0, task="aae_wgan_" + str(z_dim), ds_scale=0,
                                                    wd="./Data/selscthc")  #X_train, y_train
        ...
        for train_index, tests_index in kf.split(data):
            ...
            if epoch%2==0:
                b=0
                for (name, batch) in datasets.create_test(X_test):
