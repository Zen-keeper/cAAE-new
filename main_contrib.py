...
def train(z_dim=None, model_name=None):
    ...
    z_dim = int(z_dim)
    X_train, y_train = datasets.create_datasets(retrain=0, task="aae_wgan_" + str(z_dim),ds_scale=0)#wd = "./Data/selscthc"
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch%100==0:
            b = 0
            for batch in tl.iterate.minibatches(inputs=x_test, targets=np.zeros(x_test.shape),
