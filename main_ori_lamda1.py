...
def train(z_dim=None, model_name=None):
    ...
    with tf.device("/gpu:0"):
        ...
        #     tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(d_real), logits=d_real_logits))
    ...
    for epoch in range(EPOCHS):
        ...
        data= datasets.create_datasets(retrain=0, task="aae_wgan_" + str(z_dim), ds_scale=0,
                                                    wd="Data/AD_NC/dataNC")  #X_train, y_train./Data/selscthc
...
if __name__ == '__main__':
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"  # 使用第1块GPU（从0开始）
    # # 定义TensorFlow配置
