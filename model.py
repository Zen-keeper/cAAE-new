...
def encoder(x, z_dim, reuse=False, is_train=True):
    ...
    with tf.variable_scope("Encoder", reuse=reuse):
        ...
        w_init = tf.truncated_normal_initializer(stddev=0.01)#从截断的正态分布中输出随机值
...
def decoder(x, image_size=128,reuse=False, is_train=True):
    ...
    with tf.variable_scope("Decoder", reuse=reuse):
        ...
        # s16*s16
        z_develop = DenseLayer(inputs, int(s8/2 * s8/2 * gf_dim * 16), act=lambda x : tf.nn.leaky_relu(x, 0.2),
                               name='g_dense_z')
        ...
        # s8*s8
        res2 = ResBlockUp(conv1.outputs, s8*2, batch_size, gf_dim * 4, "gres2", reuse, is_train)
