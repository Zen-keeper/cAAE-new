...
def ResBlockDown(inputs, filters, scope_name, reuse, phase_train):
    with tf.variable_scope(scope_name, reuse=reuse):
        ...
        conv1 = BatchNormLayer(conv1, act=tf.nn.leaky_relu, is_train=phase_train,
                               gamma_init=gamma_init, name='bn1')
        print("bn1:"+conv1.name)
        conv2 = Conv2d(conv1, filters * 2, (3, 3), act=None, padding='SAME', W_init=w_init, b_init=b_init, name="conv2")
        conv2 = BatchNormLayer(conv2, act=tf.nn.leaky_relu, is_train=phase_train,
                               gamma_init=gamma_init, name='bn2')
        # conv22 = Conv2d(conv1, filters * 2, (3, 3), act=None, padding='SAME', W_init=w_init, b_init=b_init, name="conv22")
        # conv22 = BatchNormLayer(conv22, act=tf.nn.leaky_relu, is_train=phase_train,
        #                        gamma_init=gamma_init, name='bn22')
...
def ResBlockUp(inputs, input_size, batch_size, filters, scope_name, reuse, phase_train):
    with tf.variable_scope(scope_name, reuse=reuse):
        ...
        gamma_init = tf.random_normal_initializer(1., 0.02)
        input_layer = InputLayer(inputs, name='inputs')
        conv1 = DeConv2d(input_layer, filters, (3, 3), (2, 2),
                          act =None, padding='SAME',
                         W_init=w_init, b_init=b_init, name="deconv1")
        conv1 = BatchNormLayer(conv1, act=tf.nn.leaky_relu, is_train=phase_train,
                               gamma_init=gamma_init, name='bn1')
        conv2 = DeConv2d(conv1, int(filters / 2), (3, 3), (1, 1), act=None, padding='SAME',
                          W_init=w_init, b_init=b_init, name="deconv2")
        conv2 = BatchNormLayer(conv2, act=tf.nn.leaky_relu, is_train=phase_train,
                               gamma_init=gamma_init, name='bn2')

        # conv22 = DeConv2d(conv2, int(filters / 2), (3, 3), (1, 1), act=None, padding='SAME',
        #                  W_init=w_init, b_init=b_init, name="deconv22")
        # conv22 = BatchNormLayer(conv22, act=tf.nn.leaky_relu, is_train=phase_train,
        #                        gamma_init=gamma_init, name='bn22')

        conv3 = DeConv2d(input_layer, int(filters / 2), (3, 3), (2, 2), act=None,
                         padding='SAME',
