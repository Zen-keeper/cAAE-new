...
def decoder(x, image_size=128,reuse=False, is_train=True):
    ...
    with tf.variable_scope("Decoder", reuse=reuse):
        ...
        # s16*s16
        z_develop = DenseLayer(inputs, int(s16/2 * s16/2 * gf_dim * 16), act=lambda x : tf.nn.leaky_relu(x, 0.2),
                               name='g_dense_z')
        z_develop = tf.reshape(z_develop.outputs, [-1, int(s16/2), int(s16/2), gf_dim * 16])
