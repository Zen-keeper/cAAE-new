...
from funcs.resblocks_new import ResBlock, ResBlockDown, ResBlockUp
...
def decoder(x, reuse=False, is_train=True):
    ...
    with tf.variable_scope("Decoder", reuse=reuse):
        ...
        # s16*s16
        z_develop = DenseLayer(inputs, s16 * s16 * gf_dim * 16, act=lambda x : tf.nn.leaky_relu(x, 0.2),
                               name='g_dense_z')
        ...
        conv2_std = Conv2d(res_inputs, c_dim, (ft_size, ft_size), act=None, padding='SAME', W_init=w_init,
                           b_init=b_init,
                           name="g_conv2_std")
    return conv2, conv2.outputs, conv2_std.outputs
...
def discriminator(x, reuse=False):
    ...
    with tf.variable_scope("Discriminator", reuse=reuse):
        ...
        return net_h2, net_h2.outputs, logits
